...
from src.worldcup import (
    WorldCupModel,
    groups_dataframe as worldcup_groups_dataframe,
    link_fixture_lineup as link_worldcup_fixture_lineup,
    load_historical_matches as load_worldcup_historical_matches,
    load_players as load_worldcup_players,
    load_tournament_2026,
    lineup_payload_for_fixture as worldcup_lineup_payload_for_fixture,
    lineup_rating_adjustments as worldcup_lineup_rating_adjustments,
    lineups_summary as worldcup_lineups_summary,
    lineups_table as worldcup_lineups_table,
    simulate_worldcup as run_worldcup_simulation,
    teams_dataframe as worldcup_teams_dataframe,
    tournament_fixtures_dataframe as worldcup_fixtures_dataframe,
)
from src.worldcup.data import groups_from_tournament
...
def worldcup_overview(refresh: bool = False) -> Dict[str, Any]:
    tournament, fixture_source = load_tournament_2026(refresh=bool(refresh))
    groups = groups_from_tournament(tournament)
    fixture_df = worldcup_fixtures_dataframe(tournament)
    group_fixture_count = int((fixture_df["Grupo"] != "").sum()) if not fixture_df.empty else 0
    players_df, players_source = load_worldcup_players(refresh=False)
    return {
        "name": tournament.get("name", "World Cup 2026"),
        "teams": sum(len(teams) for teams in groups.values()),
        "groups": len(groups),
        "fixtures": int(fixture_df.shape[0]),
        "group_fixtures": group_fixture_count,
...
def worldcup_groups(refresh: bool = False) -> Dict[str, Any]:
    tournament, _ = load_tournament_2026(refresh=bool(refresh))
    return table_payload(worldcup_groups_dataframe(tournament), page=1, page_size=60)


def worldcup_fixtures(refresh: bool = False) -> Dict[str, Any]:
    tournament, _ = load_tournament_2026(refresh=bool(refresh))
    df = worldcup_fixtures_dataframe(tournament)
    return table_payload(df, page=1, page_size=150)


def worldcup_teams(refresh: bool = False) -> Dict[str, Any]:
    tournament, _ = load_tournament_2026(refresh=bool(refresh))
    model, history_source = build_worldcup_model(tournament, refresh=bool(refresh))
    df = worldcup_teams_dataframe(tournament, model=model)
    return {
        "teams": table_payload(df, page=1, page_size=60),
        "history_source": history_source,
    }


def worldcup_players(refresh: bool = False) -> Dict[str, Any]:
    df, source = load_worldcup_players(refresh=bool(refresh))
    return {
        "players": table_payload(df, page=1, page_size=250),
        "source": source,
    }


def worldcup_lineups(refresh: bool = False) -> Dict[str, Any]:
    tournament, source = load_tournament_2026(refresh=bool(refresh))
    df = worldcup_lineups_summary(tournament)
    return {
        "lineups": table_payload(df, page=1, page_size=100),
        "fixture_source": source,
    }


def worldcup_fixture_lineup(fixture_id: str, refresh: bool = False) -> Dict[str, Any]:
    tournament, _ = load_tournament_2026(refresh=False)
    payload = worldcup_lineup_payload_for_fixture(tournament=tournament, fixture_id=fixture_id, refresh=bool(refresh))
    return worldcup_lineup_response(payload)


def worldcup_refresh_fixture_lineup(fixture_id: str, payload: Dict[str, Any]) -> Dict[str, Any]:
    tournament, _ = load_tournament_2026(refresh=bool(payload.get("refresh_fixtures", False)))
    lineup = worldcup_lineup_payload_for_fixture(
        tournament=tournament,
        fixture_id=fixture_id,
        refresh=True,
        match_url=payload.get("match_url"),
    )
    return worldcup_lineup_response(lineup)


def worldcup_link_fixture_lineup(fixture_id: str, payload: Dict[str, Any]) -> Dict[str, Any]:
    tournament, _ = load_tournament_2026(refresh=False)
    lineup = link_worldcup_fixture_lineup(
        tournament=tournament,
        fixture_id=fixture_id,
...
def worldcup_lineup_response(payload: Dict[str, Any]) -> Dict[str, Any]:
    return {
        "lineup": payload,
        "players": table_payload(worldcup_lineups_table(payload), page=1, page_size=40),
    }


def worldcup_simulate(payload: Dict[str, Any]) -> Dict[str, Any]:
    iterations = min(max(int(payload.get("iterations") or 5000), 100), 20000)
    seed = int(payload.get("seed") or 2026)
    refresh = bool(payload.get("refresh", False))
    use_lineups = bool(payload.get("use_lineups", False))
    tournament, fixture_source = load_tournament_2026(refresh=refresh)
    model, history_source = build_worldcup_model(tournament, refresh=refresh)
    lineup_notes: List[str] = []
    if use_lineups:
        adjustments, lineup_notes = worldcup_lineup_rating_adjustments(tournament)
        if adjustments:
            model = model.adjusted(adjustments)
    result = run_worldcup_simulation(tournament=tournament, model=model, iterations=iterations, seed=seed)
    return {
        "summary": {
        ...
        "advancement": table_payload(result["advancement"], page=1, page_size=60),
        "matches": table_payload(result["matches"], page=1, page_size=100),
    }


def build_worldcup_model(tournament: Dict[str, Any], refresh: bool = False):
    groups = groups_from_tournament(tournament)
    teams = [team for group_teams in groups.values() for team in group_teams]
    history_df, history_source = load_worldcup_historical_matches(refresh=bool(refresh))
    return WorldCupModel.from_history(history_df, teams=teams), history_source
